...
) : ItemVm<ColorItem>(), ItemWithEvent<ShopProductListViewModel.Event> {
    private val events: MutableSharedFlow<ShopProductListViewModel.Event> = MutableSharedFlow()

    fun onColorClick() {
        item.checked.set(!item.checked.get())
        itemsScope.launch {
            events.emit(ShopProductListViewModel.Event.ColorClickEvent(item, position))
        }
    }
...
fun ColorVM.toRecyclerViewItem() = RecyclerViewItem(R.layout.item_shop_filters_color, this)
