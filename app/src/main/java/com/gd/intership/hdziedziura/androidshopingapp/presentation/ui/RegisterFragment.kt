...
import com.gd.intership.hdziedziura.androidshopingapp.databinding.FragmentRegisterBinding
...
class RegisterFragment : BaseFragment() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.validationEvents.collectLatest {
                    when (it) {
                        is RegisterViewModel.ValidationEvents.NameValidationEvent ->
                            validateName(it.validationResult)
                        is RegisterViewModel.ValidationEvents.EmailValidationEvent ->
                            validateEmail(it.validationResult)
                        is RegisterViewModel.ValidationEvents.PasswordValidationEvent ->
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.events.collectLatest {
                    when (it) {
                        is RegisterViewModel.Event.NavigateToLoginEvent -> navigateToLoginPage()
                        is RegisterViewModel.Event.NavigateToHomeEvent -> navigateToMainPage()
                        is RegisterViewModel.Event.InvalidCredentialsEvent -> invalidCredentials()
                    }
                }
            }
        }
    ...
    private fun invalidCredentials() {
        Toast.makeText(context, getString(R.string.register_error), Toast.LENGTH_SHORT).show()
    }
